<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div>
            ...
            <x-text-input id="email" :placeholder="'info@gmail.com'"
                class="block w-full mt-1 {{ $errors->get('email') ? 'border-red-600' : '' }}" type="email"
        ...
        <div class="mt-4">
            ...
            <div x-data="{ showPassword: false }" class="relative">
                ...
                <x-text-input x-bind:type="showPassword ? 'text' : 'password'" id="password"
                    class="block w-full mt-1  {{ $errors->get('email') ? 'border-red-600' : '' }}" name="password"
                    :placeholder="'Password'" :value="old('password')" required autocomplete="current-password" />
                <span @click="showPassword = !showPassword"
                    class="absolute z-30 text-gray-500 -translate-y-1/2 cursor-pointer top-1/2 right-4 dark:text-gray-400">
        ...
        <div class="flex items-center justify-end">
            @if (Route::has('password.request'))
                <a class="my-3 text-sm text-gray-600 underline rounded-md dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 focus:outline-none "
